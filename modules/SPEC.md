...
The structural rules below are normative. The module's content consists of its files after applying the manifest's `exclude` patterns (see [Core Fields](#core-fields)). Excluded files are not subject to module-content validation, including the quoted-import rule below. The symbolic-link restriction still applies to the source tree an engine traverses, including excluded paths; exclusions do not relax safe materialization or engine resource limits.
...
4. A quoted import in an included module file must resolve to an included file inside the same module root. An import such as `import "../shared.wdl"` that escapes the module root, or an import that targets an excluded file, makes the module invalid, even if the target file exists on disk. Engines must apply this rule to the resolved target, including local `file:` URIs and URI-encoded paths, so an alternative spelling cannot bypass exclusion.
...
- **`exclude`** (array of strings, optional). A list of gitignore-style glob patterns identifying files to omit from module content. Each pattern must be a relative path that, after resolving any `.` or `..` components, points to a location under the module root. Absolute paths (leading `/` or a Windows-style drive letter) and patterns that resolve outside the module root are not permitted. The path separator is `/`. Patterns are matched against paths relative to the module root. Plain directory names exclude the directory and everything beneath it; `*` matches any sequence of non-separator characters; `**` matches any sequence including separators. Excluded files are omitted from content hashing, signing, module-content validation, and packaged module content. Neither symbolic imports nor quoted imports from included module files may resolve to excluded files. The root `module.json` is always included, even when a pattern matches it, so the manifest and its exclusion policy remain covered by the content hash and signature. See [Symbolic Module Paths](#symbolic-module-paths) for the resolution behavior. Defaults to the empty list.

Exclusions let authors keep development files beside module files. For example, `"exclude": ["testrun.wdl", "tests"]` omits a local test harness and a test directory from the module. An escaping quoted import in that harness does not invalidate the module. A user may still run the harness directly from the source checkout under ordinary local-file import rules; excluded files are not private or inaccessible on disk. Included module files must not depend on their presence. Engines may retain excluded files in a source checkout, but must enforce the same module-content rules whether those files are present or absent.

For exclusion matching, engines must normalize both the patterns and module-relative paths to Unicode Normalization Form C (NFC). Matching is case-sensitive and anchored at the module root: `tests` matches the root `tests` directory and its descendants, while `**/tests` also matches nested directories. A path is excluded if any pattern matches it or one of its ancestor directories. Pattern order has no effect, and a leading `!` is literal rather than a negation operator. Invalid patterns are manifest errors. Engines must not infer additional exclusions from `.gitignore` files.
...
The entrypoint is the default surface, not a privacy boundary. Consumers may also import individual included files within the module folder by sub-path (see [Symbolic Module Paths](#symbolic-module-paths)); such imports do not pass through the entrypoint. The manifest's `exclude` field removes matched files from the module entirely (see [Core Fields](#core-fields)); it cannot be used to hide a helper file that included module files still import.
...
If the manifest's `exclude` field matches the path that resolution would otherwise read—either the entrypoint file for a root module import or `<sub-path>.wdl` for a sub-path import—the engine must treat the import as unresolvable and surface a missing-file error that names the path. An excluded path is not part of the module. Excluding the entrypoint therefore makes root module imports of that dependency fail while leaving non-excluded files reachable by sub-path. Quoted imports from included module files must also reject excluded targets, even when the target exists in the local source checkout.
...
2. Compute each file's relative path from the module root using `/` as the path separator, regardless of the host operating system. Normalize each relative path to Unicode Normalization Form C (NFC) before any further use. Omit files matched by the root manifest's NFC-normalized `exclude` patterns, except that the root `module.json` must always be included. If two distinct included entries normalize to the same NFC form, the module is invalid.
