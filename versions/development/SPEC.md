...
Strings that begin and end with three adjacent single- or double-quotes may span multiple lines. By default, all whitespace between the last non-whitespace character on one line and the first non-whitespace character on the next line is replaced by a single space. For example, all four strings below identical:
