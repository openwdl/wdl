...
public class WdlSourceCodeFormatter {
  ...
  private String format(CompositeTaskNode node, int indent) {
    ...
    if ( node instanceof CompositeTask ) {
      ...
      builder.append( indent_str + this.colorizer.keyword("for") + " ( " + this.colorizer.variable(variable_to_string(loop.getVariable())) + " : " + this.colorizer.variable(variable_to_string(loop.getCollection())) + " ) {\n" );
